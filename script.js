document.addEventListener('DOMContentLoaded', () => {
    ...
    function startSlotMachineInColumns() {
        ...
        const lists = window.gameLists || { jili: [], pg: [], pp: [] };

        let stackJili = getStack(carouselJili, lists.jili, 'jili');
        let stackPg = getStack(carouselPg, lists.pg, 'pg');
        let stackPp = getStack(carouselPp, lists.pp, 'pp');

        // Spin Logic - Use globally stored lists

        const p1 = spinColumn(stackJili, lists.jili, pickUniqueGames(lists.jili, 3), 2000, 'jili');
        const p2 = spinColumn(stackPg, lists.pg, pickUniqueGames(lists.pg, 3), 3000, 'pg');
        const p3 = spinColumn(stackPp, lists.pp, pickUniqueGames(lists.pp, 3), 4000, 'pp');
